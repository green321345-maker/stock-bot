...
- 필터 결과에 예상주가/업사이드 표시 + 최소 업사이드 필터
- 필터 결과 CSV 다운로드
- 기간 민감 필터(선택한 기간 수익률 기준)
...
앱에서 `alert_rules.json`을 저장한 뒤, GitHub Secret으로 바로 동기화할 수 있습니다.

```bash
./run_sync_secrets.sh owner/repo
```

내부적으로 아래 시크릿을 업데이트합니다:
- `ALERT_RULES_JSON` (필수)
- `ALERT_STATE_JSON` (선택/동기화 포함)
