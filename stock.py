...
<style>
  ...
  .block-container { padding-top: 0.8rem; padding-bottom: 1.4rem; max-width: 1120px; }
  .hero { padding: 0.85rem 1rem; border-radius: 16px; background: linear-gradient(90deg, #F7ECFF 0%, #FFF7FD 75%); border: 1px solid #EBD8FF; box-shadow: 0 4px 14px rgba(178,150,255,0.14); }
  .hero h1 { margin: 0; color: __ACCENT_DARK__; font-size: 1.65rem; letter-spacing: -0.01em; }
  .chip { display:inline-block; padding: 0.24rem 0.58rem; border-radius:999px; background:#EFE6FF; color:__ACCENT_DARK__; margin-right:0.35rem; font-size:0.74rem; }
  .soft-card { background:white; border:1px solid #EEE7FF; border-radius:12px; padding:0.75rem 0.9rem; }
  .purple { color: __ACCENT_DARK__; font-weight: 700; }
  .stMetric { transform: scale(0.94); transform-origin: left top; }
  div[data-testid="stDataFrame"] { font-size: 0.9rem; }
  .stTabs [data-baseweb="tab-list"] button { font-size: 0.92rem; }
  @media (max-width: 900px) {
    .block-container { max-width: 100%; padding: 0.6rem 0.5rem 1.2rem; }
...
@st.cache_data(ttl=300)
def load_analyst_targets(ticker: str):
    return yf.Ticker(ticker).analyst_price_targets


def period_return_pct(hist: pd.DataFrame) -> Optional[float]:
...
with st.sidebar:
    ...
    use_period_filter = st.checkbox("기간 민감 필터 사용", value=True)
    min_period_return = st.slider("최소 기간 수익률(%)", -80, 200, -10, 1)
    min_upside = st.slider("최소 예상 업사이드(%)", -50, 300, 5, 1)
    st.markdown("---")
...
if use_period_filter:
    st.caption(f"기간({period}) 기반 최소 수익률 필터 적용 중: {min_period_return}%")
st.caption(f"최소 예상 업사이드 필터 적용 중: {min_upside}%")
if st.button("전략 기준으로 후보 기업 바로 보기", type="primary"):
    ...
    for i, tk in enumerate(CANDIDATE_TICKERS):
        try:
            ...
            hist_for_period = load_history(tk, period)
            p_return = period_return_pct(hist_for_period)
            targets = load_analyst_targets(tk)
            model = reliability_estimated_price(info, targets if isinstance(targets, dict) else {})
            est_price = model["estimated_price"] if model else None
            current_price = safe_num(info.get("currentPrice"))
            upside_pct = ((est_price / current_price) - 1) * 100 if (est_price and current_price and current_price > 0) else None

            style_ok, style_reasons = style_capital_match(info, invest_style, capital_range, cap_filter)
            period_ok = True
            upside_ok = True
            if use_period_filter and p_return is not None:
                period_ok = p_return >= min_period_return
            if upside_pct is not None:
                upside_ok = upside_pct >= min_upside
            pass_checks = agg["score"] >= min_score and agg["reliability"] >= min_reliability and style_ok and period_ok and upside_ok
            ...
            if pass_checks:
                rows.append(
                    {
                        ...
                        "현재가": current_price,
                        "예상주가": round(est_price, 2) if est_price is not None else None,
                        "업사이드(%)": round(upside_pct, 2) if upside_pct is not None else None,
                        "전략점수(평균)": agg["score"],
                        "신뢰도(평균)": agg["reliability"],
                        "전략개수": len(selected_strategies),
                        "기간수익률(%)": round(p_return, 2) if p_return is not None else None,
                ...
                if use_period_filter and p_return is not None and not period_ok:
                    fail_reason.append(f"기간 수익률 미달: {p_return:.2f}% < {min_period_return}%")
                if upside_pct is not None and not upside_ok:
                    fail_reason.append(f"업사이드 미달: {upside_pct:.2f}% < {min_upside}%")
                rows.append(
                    {
                        "티커": tk,
                        "기업명": info.get("shortName") or tk,
    ...
    if not df.empty:
        ...
        if not passed_df.empty:
            ...
            st.download_button(
                ...
                file_name=f"stockbot_filtered_{period}.csv",
                mime="text/csv",
                key=f"download_filtered_{period}",
            )
        else:
        ...
        with st.expander("제외된 종목 및 사유 보기"):
            if not failed_df.empty:
                st.dataframe(failed_df[["티커", "기업명", "시총구분", "예상주가", "업사이드(%)", "통과전략", "미통과전략", "전략점수(평균)", "신뢰도(평균)", "제외사유"]], use_container_width=True)
            else:
                st.caption("제외 종목 없음")
