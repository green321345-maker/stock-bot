...
def style_capital_match(info: dict, invest_style: str, capital_range: str) -> Tuple[bool, List[str]]:
    reasons: List[str] = []
    ok = True
...
if st.button("전략 기준으로 후보 기업 바로 보기", type="primary"):
    ...
    for i, tk in enumerate(CANDIDATE_TICKERS):
        try:
            ...
            style_ok, style_reasons = style_capital_match(info, invest_style, capital_range)
                ...
                rows.append(
                    {
                        ...
                        "전략개수": len(selected_strategies),
                        "스타일/금액 적합": "부적합",
                        "제외사유": ", ".join(style_reasons) if style_reasons else "전략 점수 미달",
                        "섹터": info.get("sector") or "-",
                    }
                )
        except Exception:
    ...
    if not df.empty:
        ...
        with st.expander("제외된 종목 및 사유 보기"):
            if not failed_df.empty:
                st.dataframe(failed_df[["티커", "기업명", "전략점수(평균)", "신뢰도(평균)", "제외사유"]], use_container_width=True)
            else:
                st.caption("제외 종목 없음")
    else:
...
if selected:
    ...
    with tab4:
        ...
        account_size = st.number_input("계좌 총액", min_value=100000.0, value=5000000.0, step=100000.0)
        risk_pct = st.slider("1회 거래 허용 손실(%)", 0.5, 5.0, 1.0, 0.5)
        pos = suggest_position_size(hist, account_size, risk_pct, safe_num(info.get("currentPrice")))
        if pos["shares"] is not None:
            st.info(
                f"권장 수량: {pos['shares']}주 | 권장 진입금액: {pos['position_value']:.0f} | 추정 손절폭: {pos['stop_distance']:.2f}"
            )
